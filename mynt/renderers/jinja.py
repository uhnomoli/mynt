...
class Renderer(_Renderer):
    ...
    def _absolutize(self, html):
        def _replace(match):
            return self._get_url(
                match.group(1).replace(self.globals['site']['base_url'], '', 1), True)

        return sub(r'(?<==")({0}[^"]*)'.format(
            absurl(self.globals['site']['base_url']), ''), _replace, html)
    ...
    def from_string(self, source, vars_=None):
        if vars_ is None:
            vars_ = {}
    ...
    def render(self, template, vars_=None):
        if vars_ is None:
            vars_ = {}
        try:
